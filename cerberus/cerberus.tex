\chapter{Integrating \erratum{} into \cerberus{}}\label{chap:cerberus}
...
% 
% Section~\ref{cerberus:sec:locator} formalizes the locator problem we address in this paper.
% Section~\ref{cerberus:sec:implementation} introduces our approach, \erratum, which leverages an efficient flexible tree matching solution that we identified.
% Section~\ref{cerberus:sec:benchmark} describes the locator benchmark we designed and implemented.
% Section~\ref{cerberus:sec:evaluation} reports on the performance of our approach compared to the state-of-the-art algorithms.
% Section~\ref{cerberus:sec:threats} discusses the threats to validity of our contribution.
...
\section{Background}\label{cerberus:sec:related-work}
...
% We walk through the key steps of the SFTM algorithm we integrated into \erratum{} by using HTML snippets reported in Figure~\ref{cerberus:fig:example_html}.
...
% \paragraph{Step\,1: Node Similarity}\label{cerberus:sec:node_similarity}
...
% \begin{ex}
% The \emph{label} computed for $div_1$ (cf. Figure~\ref{cerberus:fig:example_html}) includes the following tokens: $\{\texttt{div}, \texttt{class}, \texttt{content-info\_\_item} \}$. %The algorithm ignores the content.
% \end{ex}
...
It appears that these drawbacks are more than mere inconvenience. 
...
% To help solve the problems described in the previous section, we integrated Erratum into Cerberus.
...
% Addressing the integration of \erratum through the robust locator problem rather than locator repair is a subtle pivot that eases the implementation, as it allows \erratum to be included similarly to already existing locator methods, like \textit{XPath} or \textit{CSS}.
...
\section{Industrial Impact}\label{cerberus:sec:impact}
...
\section{Perspectives}\label{cerberus:sec:perspectives}
\paragraph{Adoption}
