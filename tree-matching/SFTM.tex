...
% Indeed, the existing approaches, like \emph{Tree-Edit Distance} (TED) or \emph{Flexible Tree Matching} (FTM) fail to scale beyond hundreds of nodes, which is far below the average complexity of existing web apps.
...
% \end{inparaenum}
% A recent work by Kumar et al introduced a novel flexible tree-matching algorithm to relax this ancestry restriction of TED, but the proposed algorithm is impractical to run on trees that have more than a few hundred nodes.
% Based on Kumar et al's work, we introduce a novel flexible-tree-matching algorithm that performs in $O(n\ log(n))$ time, thus solving both limitations of TED. To do so, we approach tree matching as an optimization problem and leverage node labels and local topology similarity to avoid combinatorial explosion.
...
% For example, when it comes to HTML web pages, keeping track of all changes requires comparing \emph{Document Object Model} (DOM) trees across multiple versions of a given resource.
...
Overall, when matching two web pages, even the most efficient TED implementation~\cite{pawlik2016tree} offers far from optimal accuracy (69\% of precision on average in our empirical evaluation) for computation times often reaching several seconds.
...
% Unfortunately, state-of-the-art algorithms have been proposed to achieve this goal imposing to choose between \emph{matching accuracy} or \emph{fast matching}~\cite{xydiff} for any tuple of web documents.
% Furthermore, one can also observe that existing algorithms fail to scale beyond a few hundred nodes.
...
% % TED is a restriction of tree matching where descendants of matched nodes can only match with each other (ancestry restriction) and siblings' order must be preserved.
...
% However, when reproducing this experiment with \textsc{XyDiff},\footnote{\url{https://github.com/fdintino/xydiff}} which is a set of tools for creating and applying deltas from XML documents, results are reported much faster, but the price of poor accuracy of the resulting tree matchings.

% Figure~\ref{sftm:fig:tree_matching_example} illustrates the tree-matching problem. 
...
%     \label{sftm:fig:tree_matching_example}
% \end{figure}
...
\paragraph{\bf XML\,diff}\label{sftm:sec:xmldiff}
...
We, therefore, aim at reducing the complexity of the FTM algorithm to scale on complex web pages without enforcing restrictions on produced tree-matching solutions.
...
The upper part of Figure~\ref{sftm:fig:steps} describes the main steps involved in computing the final full matching between $T$ and $T'$.
...
\subsection{\textsc{Metropolis} Algorithm}\label{sftm:sec:metropolis_ftm}% \cite{Kumar2011_FTM}
...
% \section{Similarity-based Flexible Tree~Matching}\label{sftm:sec:SFTM}
...
To find the neighbors of nodes from $T'$ using $TM$, we iterate through all the nodes in $T'$, while each node in $T'$ has $N_{tk}$ tokens.
...
% The empirical evaluation conducted in Section~\ref{sftm:sec:evaluation} tends to suggest that the average-case complexity may be even lower in practice.
...
To the best of our knowledge, there is no established and public benchmark that includes such pairs of trees, along with the ground truth matching of their nodes. Creating such a benchmark is challenging.
...
% For this reason, we could only run the modified version of APTED on a few hundred small pages and the results presented in Figure~\ref{sftm:fig:f1score} reports on the non-modified version.
...
% Figure~\ref{sftm:fig:sftm_speed} delivers a closer look on the scalability of SFTM.
% The empirical results seem to indicate an evolution in $O(n \cdot log(n))$: in  Figure~\ref{sftm:fig:sftm_speed}, we replaced the X axis from the number of nodes in the DOM $n$ to $n\ log(n)$ then computed a linear regression on the curve which resulted in a correlation coefficient $r^2 = 86\%$.
% We believe the complexity depends on the distribution of nodes within the tokens and a more thorough theoretical analysis will be conducted in the future to understand the gap with the theoretical complexity (cf. Section~\ref{sftm:sec:complexity}). 
    ...
    %     \label{sftm:fig:sftm_speed}
    % \end{figure}
    
% This observation raises the question of the impact of the sublinear threshold function $f$ on the performance of SFTM.
% We, therefore, conducted a sensitivity analysis of this parameter to better understand potential trade-offs offered by the definition of this function, with regards to the complexity analysis we performed (cf. Section~\ref{sftm:sec:complexity}).
...
Figure~\ref{sftm:fig:parameter}, therefore, reports on the evolution of SFTM performances when $f$ varies.
...
% Nevertheless, the repetition of measures reports a clear signal in favor of SFTM.
...
%Existing methods to identify elements on a page (XPath) are limited. Notably, they do not allow the expression of \textit{flexible}, statistics-based identifiers. That's why some works have developed some more flexible identification methods [...] and applied them to repair broken locators.
...
% However, doing so would not guarantee obtaining a full matching, since we might have selected several edges linked to the same nodes.
...
% To approach the optimal matching, we use the same technique as FTM does: the metropolis algorithm~\cite{metropolis1953equation}.
% The metropolis algorithm provides a way to explore a probability distribution by random walking through samples.
% We use this algorithm for a random walk-through of several full matching and retain the least costly.
...
%     \item A parameter $N$ that defines the number of random walks to perform before returning the best-visited sample.
% \end{compactenum}
